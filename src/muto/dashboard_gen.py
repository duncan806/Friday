...
package data (muto/data/), loaded via importlib.resources and rendered into
...
def _data(name: str):
    return resources.files("muto").joinpath("data", name)
...
def generate(state, root: Path) -> Path:
    ...
    tokens = {
        ...
        "@@ALERT@@": alert,
        "@@MASCOTS@@": mascots,
        "@@TITLE_PANEL@@": _panel("", "muto—validation dashboard"),
