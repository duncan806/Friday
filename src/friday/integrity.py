"""friday integrity asserts—spec §3.
...
def assert_reverse_channel_clean(question: str) -> None:
    """Codex -> Claude round-trip questions must not carry code/How.

    When the round-trip channel is enabled (dialogue_turns>0), a Codex question
    routed to Claude is filtered by reverse_filter; if anything was dropped, the
    raw question leaked How (code/paths/diffs) and the round-trip turn is void
    (spec §8, D3 — symmetric to assert_codex_prompt_clean on the Why side).
    """
    from .reverse_filter import reverse_filter
    _, dropped = reverse_filter(question)
    if dropped:
        raise IntegrityBreach(
            f"reverse channel leaks code/How to Claude: {dropped[0][:60]!r}")
