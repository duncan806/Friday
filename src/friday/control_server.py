...
def safe_get_target(root: Path, url_path: str) -> Path | None:
    """Resolve a GET path to a real file, refusing traversal outside dashboard/.

    Only dashboard/* and status.json are served. The previous check (root in
    parents) exposed task/ and predictions/ via `/../task/task.md` — the very
    Why the system hides (review Med#4). Returns None if not allowed / missing.
    """
    ...
    if url_path == "/status.json":
        ...
        target = root / "dashboard" / url_path.lstrip("/")
    target = target.resolve()
    dash = (root / "dashboard").resolve()
    allowed = target == (root / "status.json").resolve() or target == dash or dash in target.parents
    return target if (allowed and target.is_file()) else None


def origin_ok(origin: str | None) -> bool:
    """CSRF guard: reject cross-site POSTs. A browser sends Origin; a same-site
    request carries a localhost host. Non-browser clients (no Origin) are not a
    CSRF vector and pass."""
    if not origin:
        return True
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        target = safe_get_target(self.server.root, path)
        if target is None:
    ...
    def do_POST(self):
        if not origin_ok(self.headers.get("Origin")):
            return self._reply(403, b'{"error":"cross-origin request refused"}',
                               "application/json")
        try:
    ...
    def watch():
        ...
        while True:
            if start.exists():
                ...
                pid = root / "friday.pid"
                if not pid.exists():
                    subprocess.Popen([sys.executable, "-m", "friday", "run", "--attach"],
